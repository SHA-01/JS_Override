(function(wHandle, wjQuery) {
    var CONNECTION_URL = "cubedot.mooncp.net:12081", // Default Connection
    ...
    function DrawMini() {
        ...
        var NDX = ''
        var NDY = ''
        ctx.fillText(NDX, TouWidth + 20, Touheight - 10);
        ctx.fillText(NDY, TouWidth + 40 + ctx.measureText(NDX).width, Touheight - 10);
        ctx.closePath();
        ...
        for (var NDX = 0; NDX <= Toucha; NDX += 40) {
            ...
            var ko = ["A", "B", "C", "D", "E"];
            for (var i = 0; i < 5; i++) {
                ctx.fillText(ko[i] + NDX / 40, (0.5 + NDX + TouWidth) - 20, Touheight + 25.5 + (i * 40))
            }
        }
        ctx.globalAlpha = 1;
    ...
    function viewRange() {
        var ratio;
        ratio = Math.max(canvasHeight / 6480, canvasWidth / 11520);
        return ratio * zoom;
    }
    ...
    function drawGrid() {
        ctx.fillStyle = showDarkTheme ? "#1C1C1C" : "#F2FBFF";
        ctx.fillRect(0, 0, canvasWidth, canvasHeight);
        ctx.save();
    ...
    function drawLeaderBoard() {
        ...
        if (drawTeam || 0 != leaderBoard.length)
            if (drawTeam || showName) {
                ...
                ctx.fillStyle = "#FFFFFF";
                var c = "Leaderboard";
                ctx.font = "28px Nanum Gothic";
                ctx.fillText(c, 100 - ctx.measureText(c).width * 0.5, 40);
                var b, l;
                if (!drawTeam) {
                    for (ctx.font = "16px Nanum Gothic", b = 0, l = leaderBoard.length; b < l; ++b) {
                        ...
                        me ? ctx.fillStyle = "#F7FE2E" : ctx.fillStyle = "#FFFFFF";
    ...
    var nCanvas, ctx, mainCanvas, lbCanvas, chatCanvas, canvasWidth, canvasHeight, qTree = null,
        ...
        ejectIcon = new Image,
        noRanking = false;
    splitIcon.src = "http://cubedot.kr/play/assets/img/split.png";
    ejectIcon.src = "http://cubedot.kr/play/assets/img/feed.png";
    ...
    wHandle.openSkinsList = function(arg) {
        if ($('#inPageModalTitle').text() != "스킨") {
            $.get('include/gallery.php').then(function(data) {
                $('#inPageModalTitle').text("스킨");
